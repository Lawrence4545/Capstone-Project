resource "null_resource" "get_nlb_hostname" {
    provisioner "local-exec" {
        command = "aws eks update-kubeconfig --name law-dev-eks --region us-west-1 && kubectl get svc load-nginx  --namespace nginx-ingress -o jsonpath='{.status.loadBalancer.ingress[*].hostname}' > ${path.module}/lb_hostname.txt"
    }
    depends_on = [
...
locals {
  instances = {
    namea = "capstone-sock-shop.${var.domain_name}"
    nameb = "grafana.${var.domain_name}"
    named = "result.${var.domain_name}"
  }
}
